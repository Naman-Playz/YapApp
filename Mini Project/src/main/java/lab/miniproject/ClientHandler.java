...
//        }
    ...
    private void createDM(String email, String receiverEmail) throws SQLException {
        boolean dmexists = db.DMExists(email, receiverEmail);
        if (dmexists){
            // load past chats
        } else{
        boolean result = db.createDM(email, receiverEmail);
        try{
            if(result){
                bufferedWriter.write("SUCCESS");
                bufferedWriter.newLine();
